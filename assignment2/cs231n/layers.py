...
def batchnorm_forward(x, gamma, beta, bn_param):
  ...
  if mode == 'train':
    ...
    cache = (mode, x, gamma, xc, std, x_norm, sample_mean, sample_var, eps)
    ...
    cache = (mode, x, x_norm, gamma, beta)
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
  else:
...
def batchnorm_backward(dout, cache):
  ...
  mode = cache[0]
  if mode == 'train':
        mode, x, gamma, xc, std, xn, sm, sv ,eps= cache
        N = x.shape[0]
...
def batchnorm_backward_alt(dout, cache):
  ...
  #############################################################################
  mode, x, gamma, xc, std, xn, sample_mean, sample_var ,eps = cache
  m = dout.shape[0] # m is N here
  dxn = dout * gamma # (N, D)
  dvar = (dxn * (x-sample_mean) * (-0.5) * np.power(sample_var+eps, -1.5)).sum(axis = 0)  # (D,)
  dmean = np.sum(dxn * (-1) * np.power(sample_var + eps, -0.5), axis = 0)
  dmean += dvar * np.sum(-2 * (x - sample_mean), axis = 0) / m
  dx = dxn * np.power(sample_var + eps, -0.5) + dvar*2*(x - sample_mean) / m + dmean / m
...
def conv_forward_naive(x, w, b, conv_param):
  ...
  for i in arange(output_height):
    for j in arange(output_width):
...
def conv_backward_naive(dout, cache):
  ...
  for i in arange(output_height):
    for j in arange(output_width):
        ...
        for n in range(N):
            dx_pad[n,:,i*stride:i*stride+HH, j*stride:i*stride+WW] += np.sum((w[:, :, :, :] *
...
def max_pool_forward_naive(x, pool_param):
  ...
  #############################################################################
  ph, pw, stride = pool_param['pool_height'] ,pool_param['pool_weight'], pool_param['stride']
  N, C, H, W = x.shape
  output_height = H / ph
  output_width = W / pw
  ...
  for i in arange(output_height):
    for j in arange(output_width):
        x_mask = x[:,:,i*stride:i*stride+ph, j*stride:j*stride+WW]
...
def max_pool_backward_naive(dout, cache):
  ...
  for i in arange(output_height):
    for j in arange(output_width):
        x_pool = x[:,:,i*ph:i*ph+ph, j*pw:j*pw+pw]
        max_idx = np.unravel_index(np.argmax(x_pool), x_pool.shape)
        dx[:,:,i*ph+max_idx[0],j*pw+max_idx[1]) = dout[:,:,i,j]
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
  return dx
