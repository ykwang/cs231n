...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None):
    ...
    affine2_dx,affine2_dw2,affine2_db2=affine_backward(dscores, affine2_cache)
    grads['W2'] = affine2_dw2 + self.reg*self.params['W2']
    grads['b2'] = affine2_db2
    
    affine1_dx,affine1_dw1,affine1_db1 = affine_relu_backward(affine2_dx,affine1_relu_cache)
    grads['W1'] = affine1_dw1 + self.reg*self.params['W1']
    grads['b1'] = affine1_db1
...
class FullyConnectedNet(object):
  ...
  def loss(self, X, y=None):
    ...
    # of 0.5 to simplify the expression for the gradient.                      #
    ############################################################################
    data_loss, grads_out = softmax_loss(scores, y)
    
    reg_loss = 0
    
    layer_grad = grads_out
    ...
    for layer in xrange(self.num_layers, 0, -1):
        ...
        if(layer == self.num_layers):
            ...
            if(self.use_batchnorm):
                dx,dw,db,dgramma,dbeta = affine_batchnorm_relu_backward(layer_grad, inputs[layer-1])
            else:
                if(self.use_dropout):
